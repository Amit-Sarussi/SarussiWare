...
<template>
  ...
  <section class="db-section">
    ...
    <template v-else>
      <ul class="user-list">
        <li v-for="u in users" :key="u.id">{{ u.name }} <small>(id: {{ u.id }})</small></li>
      </ul>
...
<style scoped>
.db-section { margin: 2rem; padding: 1rem; border: 1px solid #ddd; border-radius: 8px; }
.db-section h2 { margin-top: 0; }
.error { color: #c00; }
.user-list { list-style: none; padding-left: 0; }
.add-user { margin-top: 1rem; display: flex; gap: 0.5rem; }
.add-user input { flex: 1; padding: 0.25rem 0.5rem; }
</style>
