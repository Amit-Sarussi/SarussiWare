...
const account = computed(() => {
	...
	return id ? getInvestmentAccount(id) : undefined;
});
...
watch(
	account,
	(acc) => {
		if (route.name === "finance-investment-account-detail" && acc === undefined && route.params.id) {
			router.replace({ name: "finance-manage-investment-accounts" });
		}
	},
	{ immediate: true }
);
...
const currentBalance = computed(() => {
	const id = route.params.id as string;
	return id ? (balancesNow.value.investmentAccount[id] ?? 0) : 0;
});

const balanceEndOfMonth = computed(() => {
	const id = route.params.id as string;
	return id ? (balancesAfterThisMonth.value.investmentAccount[id] ?? 0) : 0;
});
...
const currentMonthChartData = computed(() => {
	...
	const balanceAtStart = getBalancesAsOf(start).investmentAccount[id] ?? 0;
	...
	return {
		...
		actualData: timePoints.map((d) => getBalancesAsOf(d).investmentAccount[id] ?? 0),
	};
});
...
const editYearlyRate = ref<number>(0);
const editError = ref("");
...
function openEdit() {
	if (account.value) {
		...
		editPercentage.value = account.value.percentage;
		editYearlyRate.value = account.value.yearlyRate;
		editError.value = "";
...
async function saveEdit() {
	...
	if (!Number.isFinite(rawPct) || rawPct < 0 || rawPct > 100) {
		editError.value = "Enter a percentage between 0 and 100.";
		return;
	}
	const pct = Math.round(Math.max(0, Math.min(100, rawPct)) * 100) / 100;
	try {
		await updateInvestmentAccount(id, editName.value.trim(), pct, editYearlyRate.value);
		closeEdit();
	} catch (e) {
...
<template>
	<div v-if="account" class="cider cider-fixed max-w-4xl">
		...
		<div v-if="showEditForm" class="card p-5 mb-6">
			<h2 class="text-[15px] font-semibold mb-3">Edit investment account</h2>
			<div class="flex flex-wrap items-end gap-3">
				<div class="min-w-0">
					<label class="block text-[13px] text-tertiary-foreground mb-1">Name</label>
					<input v-model="editName" type="text" class="w-48" placeholder="e.g. Index fund" />
				</div>
				<div class="min-w-0">
				...
				<div class="min-w-0">
					<label class="block text-[13px] text-tertiary-foreground mb-1">Yearly rate %</label>
					<input
						:value="editYearlyRate"
						type="text"
						inputmode="decimal"
						class="w-24 text-right"
						placeholder="5"
						@input="(e) => { editYearlyRate = parseRate((e.target as HTMLInputElement).value); }" />
				</div>
				<span class="text-tertiary-foreground text-[15px] pb-2">%/yr</span>
		...
		<p class="text-tertiary-foreground text-[15px] mt-1">
			{{ account.percentage }}% of net income each month, with {{ account.yearlyRate }}% yearly growth applied monthly (12th root). Projected over 24 months.
		</p>
		...
		<div class="grid gap-4 mt-6 sm:grid-cols-2">
			...
			<div class="card p-5">
				...
				<p class="text-[13px] text-tertiary-foreground mt-1">
					Balance at end of current month (this month’s allocation + growth applied)
				</p>
			</div>
		</div>
